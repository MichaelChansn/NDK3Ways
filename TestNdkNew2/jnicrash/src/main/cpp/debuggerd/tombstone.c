...
static void dump_system_info(log_t* log)
{
    ...
    if ((fp = fopen(path, "r"))) {
    	cpuinfo = fgets(infobuf, sizeof(infobuf), fp);
        fclose(fp);
        if (cpuinfo) {
            size_t len = strlen(cpuinfo);
            if (len && cpuinfo[len - 1] == '\n') {
            	cpuinfo[len - 1] = '\0';
            }
        }
    }
    ...
    path = "/proc/version";
    if ((fp = fopen(path, "r"))) {
    	sysinfo = fgets(infobuf, sizeof(infobuf), fp);
        fclose(fp);
        if (sysinfo) {
            size_t len = strlen(sysinfo);
            if (len && sysinfo[len - 1] == '\n') {
            	sysinfo[len - 1] = '\0';
            }
        }
    }
    _LOG(log, 0, "%s\n", sysinfo ? sysinfo : "UNKNOWN");
...
//             (uintptr_t) si.si_addr);
        _LOG(log, SCOPE_AT_FAULT, "signal %d (%s)\n",
             sig, get_signame(sig));
    } else {
...
static void dump_backtrace_and_stack(const ptrace_context_t* context, log_t* log, pid_t tid,
    ...
    ssize_t frames = unwind_backtrace_ptrace(tid, context, backtrace, 0, STACK_DEPTH, at_fault);
    if (frames > 0) {
...
static void dump_maps(pid_t tid, log_t* log) {
	_LOG(log, 0, "\nmaps:\n");
	map_info_t* map = load_map_info_list(tid);
	map_info_t* head = map;
	...
	while (map != NULL) {
	    if (map != NULL && strlen(map->name) > 0) {
	    	if ((map->next == NULL || strcmp(map->name, map->next->name) != 0)) {
	    		_LOG(log, 0, "    %08x-%08x %s\n", map->start, map->end, map->name);
	    	} else {
	    		start = map->start;
	    		end = map->end;
	    		while (map->next != NULL && (strcmp(map->name, map->next->name) == 0)) {
	    			map = map->next;
	    			if (map->start < start) {
	    				start = map->start;
	    			}
...
static bool dump_sibling_thread_report(const ptrace_context_t* context,
    ...
    while ((de = readdir(d)) != NULL) {
        ...
        dump_thread(context, log, new_tid, false);
...
//    }
    ...
    dump_thread(context, log, tid, true);
...
//    }
    ...
    // 其它线程不打印
//    bool detach_failed = false;
//    detach_failed = dump_sibling_thread_report(context, log, pid, tid);
...
bool engrave_tombstone(pid_t pid, pid_t tid, int sig, uintptr_t abort_msg_address,
                       const struct ucontext* const uc, const char* path) {
    // get crash thread registers
    get_regs_common(uc);
...
//    log.amfd = activity_manager_connect();
    log.quiet = true;
    bool result = dump_crash(&log, pid, tid, sig, abort_msg_address);
